...
│   │   ├── Combat.ts        # simulateProjectile/simulatePiercer (vuelo tick a tick) + resolveKnockback (settle server-side) + settleOnGround (gravedad para quien queda flotando sin recibir el golpe, ver abajo)
...
- Comandos: `npm run dev` (desarrollo, con recarga vía `tsx watch`), `npm run build && npm start` (producción), `npm run typecheck` (solo `tsc --noEmit`), `npm run smoke` (test de regresión Colyseus, ver abajo), `npm run smoke:api` (test de regresión de la API REST), `npm run smoke:persistence` (sobrevivir un reinicio del proceso), `npm run smoke:gravity` (jugadores no se quedan flotando cuando el terreno bajo sus pies desaparece).
...
**Predicción de proyectil en el cliente (2026-08-24):** el disparo se sigue resolviendo de una vez en `executeShot()` (nunca por ticks en el servidor — sigue sin ser necesario para un juego por turnos), pero ahora `Match` guarda el último disparo (`lastShot`: `shooterId`, `weaponId`, `dx`, `dy`, `wind` en el momento del disparo) y un contador `shotSeq`, expuestos en `toJSON()`. Esto le da al cliente Godot lo que necesita para animar la trayectoria exacta con sus propias constantes (mismo `POWER_SCALE`/gravedad/rebote/túnel que `sim/`) en vez de mostrar el resultado de golpe — ver `pawfare-client/CLAUDE.md`, sección de integración online.

**Gravedad tras perder el suelo (2026-08-24):** `resolveKnockback` solo actúa sobre un jugador si está dentro de `explosionRadius + 20` del centro del disparo — fuera de ese rango, no lo empuja ni le quita vida. Eso dejaba un bug real: si el terreno bajo un jugador desaparecía sin que él estuviera lo bastante cerca del punto de impacto (el caso típico es el túnel del Perforador, que carva terreno en muchos puntos a lo largo de su vuelo, no solo en su explosión final), el jugador se quedaba flotando sobre el cráter porque nada corregía su posición Y. `settleOnGround(player, heights)` (`Combat.ts`) es la corrección: si el jugador quedó por encima del nuevo nivel del terreno en su propia X, lo baja exactamente a ese nivel — sin desplazamiento horizontal ni rebote, es gravedad, no un empujón. Se llama desde `applyExplosionAt()` en `Match.ts` para cualquier jugador al que `resolveKnockback` NO le aplicó (el `continue` del bucle existente cubre el caso "sí lo tocó"; el nuevo bloque cubre "no lo tocó pero puede haber quedado flotando"). El cliente Godot tiene el mismo fix en paralelo (`Player._physics_process()`, ver `pawfare-client/CLAUDE.md`) para el modo local, que simula su propia física sin depender del servidor. Validado con `npm run smoke:gravity` (test determinista, sin depender de RNG de disparo: carva terreno bajo un jugador fijo y confirma que `settleOnGround` lo detecta y corrige, y que un jugador ya apoyado no recibe ninguna corrección de más).

Mismo bug, mismo fix, para las **rocas** (pregunta del usuario tras el fix de jugadores — acertada, el bug era idéntico): `applyExplosionAt()` ahora también recorre `this.obstacles` llamando `settleOnGround()` sobre cada una. `settleOnGround` no le importa el tipo de objeto, solo necesita `x`/`y`/`health` — `SpawnedRock` los tiene, así que no hizo falta ninguna función nueva, solo el segundo bucle. Cubierto por el mismo `npm run smoke:gravity` (caso extra con una roca).

**Pendiente / rough edges:** `PawfareRoom`/Colyseus queda sin cliente propio por ahora — decidir si vale la pena mantenerlo (útil si algún día hay cliente web) o retirarlo para no duplicar la orquestación de sala. La persistencia es a archivos JSON en disco local, no a una base de datos real — suficiente para sobrevivir un reinicio del proceso en esta etapa, pero no pensada para múltiples instancias del servidor en paralelo (eso sí necesitaría Postgres/Redis compartido).
